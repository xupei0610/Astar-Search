...
def main(argv):
  try:
    opts, args = getopt.getopt(argv[1:], 'hvec:s:af:t:', ['help', 'version', 'experiment', 'cases=', 'size=', 'analyze', 'file=', 'timeout='])
...
def usage():
  ...
  print('\nThe file sptest.sample.log attached with this file is a sample log generated during a test conducted on Oct.17th. It can be used to analyze, and demonstrate 5 conclusions we expect to reach via the test.')
  ...
  print('Metrics:')
  print('  1. The amount of nodes generated during solving a puzzle will be used to measure the time and space complexity.')
  print('  2. The length of the solution found by an algorithm will be used to measure the optimality of that algorithm.')
  print('  3. The running time for solving a puzzle will be shown, and it, in theory, should be directly proportional to the time complexity and space complexity.')
  print('\nUsage:')
  print('  -h, --help: show this help information.\n')
  print('  -v, --version: show the current version of this file and the Astar and SlidingPuzzle library it uses.\n')
  print('  -e, --experiment: doing test according to given parameters.\n    E.G. \'' +__file__+ ' -e -c 10 -s 3\' will generate 10 3x3 sliding puzzles for test.\n')
...
def version():
  ...
  print('%s v%s %s' % ((os.path.split(Astar.__file__))[1], Astar.__version__, Astar.__date__))
  print('%s v%s %s' % ((os.path.split(SlidingPuzzle.__file__))[1], SlidingPuzzle.__version__, SlidingPuzzle.__date__))
...
def analyze(log_file):
  ...
  min22 = 0
  max22 = 0
  optimal = {'A*-squaredEuclid': 0, 'Bi-A*-manhattan': 0, 'Bi-A*-straightLine': 0, 'Bi-A*-squaredEuclid': 0, 'GBFS-manhattan': 0, 'GBFS-straightLine': 0, 'GBFS-squaredEuclid': 0}
  optimal1 = {'A*-squaredEuclid': 0, 'Bi-A*-manhattan': 0, 'Bi-A*-straightLine': 0, 'Bi-A*-squaredEuclid': 0, 'GBFS-manhattan': 0, 'GBFS-straightLine': 0, 'GBFS-squaredEuclid': 0}
  steps = {}
  steps_to_nodes = {'A*-straightLine':{}, 'A*-manhattan':{}, 'A*-squaredEuclid':{}, 'Bi-A*-straightLine': {}, 'Bi-A*-manhattan': {}, 'Bi-A*-squaredEuclid': {}, 'GBFS-straightLine': {}, 'GBFS-manhattan': {}, 'GBFS-squaredEuclid': {}}

  for c in range(cases):
    for x in optimal:
      if s_data[x][c][0] == s_data['A*-manhattan'][c][0]:
        optimal[x] = optimal[x] + 1
        optimal1[x] = optimal1[x] + s_data[x][c][1]

    for x in steps_to_nodes:
      if s_data['A*-manhattan'][c][0] in steps_to_nodes[x]:
        steps_to_nodes[x][s_data['A*-manhattan'][c][0]][1] = steps_to_nodes[x][s_data['A*-manhattan'][c][0]][1] + s_data[x][c][1]
        steps_to_nodes[x][s_data['A*-manhattan'][c][0]][2] = steps_to_nodes[x][s_data['A*-manhattan'][c][0]][2] + s_data[x][c][0]
        steps_to_nodes[x][s_data['A*-manhattan'][c][0]][0] = steps_to_nodes[x][s_data['A*-manhattan'][c][0]][0] + 1
      else:
        steps_to_nodes[x][s_data['A*-manhattan'][c][0]] = [1, s_data[x][c][1], s_data[x][c][0]]
      
    if s_data['A*-manhattan'][c][0] in steps:
      steps[s_data['A*-manhattan'][c][0]] = steps[s_data['A*-manhattan'][c][0]] + 1
    else:
      steps[s_data['A*-manhattan'][c][0]] = 1

    if s_data['A*-manhattan'][c][1] < s_data['A*-straightLine'][c][1]:
      h = h + 1      
    elif s_data['A*-manhattan'][c][1] == s_data['A*-straightLine'][c][1]:
      i = i + 1
    else:
      j = j + 1
      
    if s_data['A*-manhattan'][c][1] < s_data['A*-squaredEuclid'][c][1]:
      h1 = h1 + 1
    elif s_data['A*-manhattan'][c][1] == s_data['A*-squaredEuclid'][c][1]:
      i1 = i1 + 1
    else:
      j1 = j1 + 1
      
    if s_data['A*-manhattan'][c][1] < s_data['Bi-A*-manhattan'][c][1]:
      h2 = h2 + 1
      if min21 == 0 or s_data['Bi-A*-manhattan'][c][0] < min21:
        min21 = s_data['Bi-A*-manhattan'][c][0]
      if max21 <  s_data['Bi-A*-manhattan'][c][0]:
        max21 =  s_data['Bi-A*-manhattan'][c][0]
    elif s_data['A*-manhattan'][c][1] == s_data['Bi-A*-manhattan'][c][1]:
      i2 = i2 + 1
    else:
      if min22 == 0 or s_data['Bi-A*-manhattan'][c][0] < min22:
        min22 = s_data['Bi-A*-manhattan'][c][0]
      if max22 <  s_data['Bi-A*-manhattan'][c][0]:
        max22 =  s_data['Bi-A*-manhattan'][c][0]
      j2 = j2 + 1
    
    if s_data['A*-manhattan'][c][0] == s_data['A*-squaredEuclid'][c][0]:
      if s_data['A*-manhattan'][c][1] < s_data['A*-squaredEuclid'][c][1]:
        h3 = h3 + 1
      elif s_data['A*-manhattan'][c][1] < s_data['A*-squaredEuclid'][c][1]:
        i3 = i3 + 1
  ...
  print('\nComparison of the amount nodes generated by A* Manhattan to A* Straight Line:')
  print('Manhattan\tthe same\tStraight Line')
  print('fewer\t                \t fewer')
  print('%d       \t%d        \t%d' % (h, i, j))
  
  
  print('\nComparison of the amount nodes generated by A* Manhattan to A* Squared Euclidean Distance:')
  print('Manhattan\tthe same\tSquared Euclidean Distance')
  print('fewer\t                \t fewer')
  print('%d       \t%d        \t%d' % (h1, i1, j1))

  print('\nComparison of the amount nodes generated by A* Manhattan to Bi-A* Manhattan:')
  print('A*\tthe same\tBi-A*')
  print('fewer\t                \t fewer')
  print('%d       \t%d        \t%d' % (h2, i2, j2))
  print('Steps Range:')
  print('%d - %d  \t           \t%d - %d' % (min21, max21, min22, max22))

  print('\nTimes to find the optimal solution:')
  for x in optimal:
    print('%s: %d, average nodes generated: %f'%(x, optimal[x], optimal1[x]/optimal[x]))
  
  title = ''
  for x in ('A*-straightLine', 'A*-manhattan', 'A*-squaredEuclid', 'Bi-A*-straightLine', 'Bi-A*-manhattan', 'Bi-A*-squaredEuclid', 'GBFS-straightLine', 'GBFS-manhattan', 'GBFS-squaredEuclid'):
    title = title + ' ' + x
  print('\nSteps v.s. Nodes Generated for')
  print('\t' + title)
  print('Steps of optimal solution ')
  for s in steps_to_nodes['A*-manhattan']:
    p = str(s)
    for x in ('A*-straightLine', 'A*-manhattan', 'A*-squaredEuclid', 'Bi-A*-straightLine', 'Bi-A*-manhattan', 'Bi-A*-squaredEuclid', 'GBFS-straightLine', 'GBFS-manhattan', 'GBFS-squaredEuclid'):
      p = p + '\t' + str(int(steps_to_nodes[x][s][1]/steps_to_nodes[x][s][0]))
    print(p)

  title = ''
  for x in ('A*-squaredEuclid', 'Bi-A*-straightLine', 'Bi-A*-manhattan', 'Bi-A*-squaredEuclid', 'GBFS-straightLine', 'GBFS-manhattan', 'GBFS-squaredEuclid'):
    title = title + '\t' + x
  print('\nSteps of Solution as the steps of optimal solutions increase')
  print(title)
  print('Steps of optimal solution')
  for s in steps_to_nodes['A*-manhattan']:
    p = str(s)
    for x in ('A*-squaredEuclid', 'Bi-A*-straightLine', 'Bi-A*-manhattan', 'Bi-A*-squaredEuclid', 'GBFS-straightLine', 'GBFS-manhattan', 'GBFS-squaredEuclid'):
      p = p + '\t%.2f' % float(steps_to_nodes[x][s][2]/steps_to_nodes[x][s][0])
    print(p)
    
  print('\nComparison of nodes generated by A* Manhattan to those by A* Squared Euclidean Distance in the case when A* Squared Euclidean Distance found optimal solutions')
  print('Manhattan Fewer\tSame\tSquaredEuclid Fewer')
  print('%d\t\t%d\t%d' % (h3, i3, j3))
  
  print('\n\nAll data have been analyzed. For the details of the results of this analysis, please see the experiment section in the author\'s paper for this experiment')
